...
def show_cat(cat_article, page):
    try:
        ...
        return render_template('components/erreur.html', erreur="La catégorie entrée est invalide !")

    data = select(a for a in Articles if cat_article in a.category).order_by(desc(Articles.timestamp))[page*10:page*10+10]
    if data == []:
    ...
    return render_template('list.html', template="Catégorie", type=cat_article, data=data_dict, icon=icon, page=page)
